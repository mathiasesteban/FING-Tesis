...
def generar_corpus_7(path_data, path_aggregate, consumo_standby, interval_size, escribir_archivo, normalizar):
    ...
    while not fin:
        ...
        # **********************  GENERACION VECTORES **********************
        vector_entrada = \
            [estado_ant, seconds_past, seconds_past_on, consumo_agregado - consumo_anterior, dia_medicion, hora_dia]
    ...
    if escribir_archivo:
        ...
        file = open(file_name + "_generar_corpus_7", "w+")
...
def print_stats(vectores_entrada, predicciones_entrada):

    len_x = len(vectores_entrada)
    dim_x = len(vectores_entrada[0])

    len_y = len(predicciones_entrada)
    dim_y = len(predicciones_entrada)

    print("Tamaño entrada: " + str(len_x))
    print("Dimension entrada: " + str(dim_x))

    print("Tamaño salida: " + str(len_y))
    print("Dimension salida: " + str(dim_y))


if __name__ == "__main__":
    _path_to_generate = "datasets/ukdale/house_2/channel_14.dat_80"
    ...
    _vectores_entrada, _predicciones_entrada = generar_corpus_7(_path_to_generate, _path_aggregate, _consumo_standby, _interval_size, _escribir_archivo, _normalizar)

    print_stats(_vectores_entrada, _predicciones_entrada)
