import numpy as np
import os
import json
import tensorflow as tf
import sys

from tensorflow import keras
from metrics import *
from keras import optimizers
from data_analisis import generador_estados
from sklearn.utils import class_weight
from sklearn import preprocessing
from keras.layers.convolutional import Conv1D
from keras.layers.convolutional import MaxPooling1D


def build_vectors_secuence(time_steps, data):
	# Elimino vectores hasta obtener un multiplo de "time_steps"
	es_multiplo = False
	while not es_multiplo:
			if len(data) % time_steps == 0:
				es_multiplo = True
			else:
				del data[0]
	
	secuence_vec = [] 
	for index, value in enumerate(data):
		aux = []
		if len(data) < time_steps + index:
			break
			
		for i in range(time_steps):
			aux.append(data[index + i])
		secuence_vec.append(aux)
	
	return np.array(secuence_vec)

def build_vectors_out_secuence(time_steps, data):
	if time_steps > 1:
		# Elimino vectores hasta obtener un multiplo de "time_steps"
		es_multiplo = False
		while not es_multiplo:
				if len(data) % time_steps == 0:
					es_multiplo = True
				else:
					del data[0]

		secuence_out_ag = data[time_steps -1:]
		return np.array(secuence_out_ag)
	else:
		return np.array(data)

def build_vectors(train_json_path,test_json_path, time_steps):
	# Datos de entrenamiento
	file1 = open(train_json_path)
	dataset_json = file1.read()
	dataset_dic = json.loads(dataset_json)
	x_train = dataset_dic["features"]
	y_train = dataset_dic["activations"]

	# Datos de testeo/validacion
	file2 = open(test_json_path)
	dataset_json2 = file2.read()
	dataset_dic2 = json.loads(dataset_json2)
	x_test_data = dataset_dic2["features"]
	y_test_data = dataset_dic2["activations"]

	# Genero conjunto de testeo/validacion
	x_val = x_test_data[:len(x_test_data)//2]
	x_test = x_test_data[len(x_test_data)//2:]

	y_val = y_test_data[:len(x_test_data)//2]
	y_test = y_test_data[len(x_test_data)//2:]

	# Normalizacion de features	
	# MaxAbsScaler -> escala en el rango [-1,1] y divide por el valor maximo de cada feature
	scaler = preprocessing.MaxAbsScaler()
	x_train_scaled = scaler.fit_transform(x_train)
	x_val_scaled = scaler.transform(x_val)
	x_test_scaled = scaler.transform(x_test)

	# Formato que entiende como entrada la red
	x_train = build_vectors_secuence(time_steps, x_train_scaled.tolist())
	y_train = build_vectors_out_secuence(time_steps, y_train)	
	x_test = build_vectors_secuence(time_steps, x_test_scaled.tolist())
	y_test = build_vectors_out_secuence(time_steps ,y_test)		
	x_val = build_vectors_secuence(time_steps, x_val_scaled.tolist())
	y_val = build_vectors_out_secuence(time_steps ,y_val)

	return x_train,y_train,x_val,y_val,x_test,y_test

def run_lstm_states(train_json_path, test_json_path, time_steps, path_to_application, consumo_standby):

	print(" LSTM ESTADOS START ")

	x_train, y_train, x_val, y_val, x_test, y_test = build_vectors(train_json_path, test_json_path, time_steps)

	states = generador_estados(path_to_application, consumo_standby)
	classification_states = len(states)
	
	# Parametros 
	epochs = 20
	optimizer = 'adam'
	loss = 'sparse_categorical_crossentropy'
	metrics = ['accuracy']
	arquitecture_type = 'CNN LSTM'
	
	print("Train path: " + train_json_path)
	print("Test path: " + test_json_path)
	print("Timesteps: " + str(time_steps))
	print("Epochs: " + str(epochs))
	print("Optimizer: " + optimizer)
	print("Loss: " + loss)
	print("Metrics: " + str(metrics))
	print("Consumo Standby: " + str(consumo_standby))
	print('Cantidad de estados: ' + str(classification_states))
	print('Tipo de arquitectura: ' + arquitecture_type)

	print("Se entrena con un conjunto de datos de tamaño: " + str(len(x_train)))
	print("Se valida con un conjunto de datos de tamaño: " + str(len(x_val)))
    print("Se testea con un conjunto de datos de tamaño: " + str(len(x_test)))

	# Reshape datos
	sub_seq = 2
	time_steps_sub_seq = 3
	features = x_test.shape[2]
	x_train = x_train.reshape((x_train.shape[0], sub_seq, time_steps_sub_seq, features))
	x_test = x_test.reshape((x_test.shape[0], sub_seq, time_steps_sub_seq, features))

	# Modelo CNN LSTM
	model = keras.Sequential()
	# CNN
	model.add(keras.layers.TimeDistributed(keras.layers.Conv1D(filters=64, kernel_size=1, activation=tf.nn.relu), input_shape=(None, time_steps_sub_seq, features)))
	model.add(keras.layers.TimeDistributed(keras.layers.MaxPooling1D(pool_size=2)))
	model.add(keras.layers.TimeDistributed(keras.layers.Flatten()))
	# LSTM 
	model.add(keras.layers.LSTM(128, activation=tf.nn.relu))
	model.add(keras.layers.Dense(classification_states, activation=tf.nn.softmax))

	# Se definen las funciones de costo, el optimizador y las metricas del modelo	
	model.compile(optimizer=optimizer, 
              	  loss=loss,
              	  metrics=metrics)
	
	model.fit(x_train, y_train, epochs=epochs, validation_data=(x_val, y_val))
	
	# Se realizan las predicciones sobre el conjunto de testeo
	predictions = model.predict(x_test)

	y_pred = []
	for predict in predictions:
	 	y_pred.append(np.argmax(predict))

	imprimir_metricas_estados(y_test,y_pred, states)

	print(" LSTM ESTADOS END ")

if __name__ == '__main__':
	train_json_path = "datasets/ukdale/house_1/channel_12.dat_80_generar_corpus_5"
	test_json_path = "datasets/ukdale/house_1/channel_12.dat_20_generar_corpus_5"
	path_to_application = "datasets/ukdale/house_1/channel_12.dat"
	time_steps = 10
	consumo_standby = 50
	run_lstm_states(train_json_path, test_json_path, time_steps, path_to_application, consumo_standby)	
